...
class WikiWindow (VimWindow):
    ...
    def on_create(self):
        ...
        vim.command('cnoremap <buffer> q<cr> python trac.normal_view()<cr>')
        vim.command('cnoremap <buffer> wq<cr> python trac.save_wiki('')<cr>python trac.normal_view()<cr>')
        ...
        vim.command('cnoremap <buffer> w<cr> TWSave<cr>')
...
class WikiVimDiffWindow (NonEditableWindow):
    ...
    def on_create(self):
        vim.command('nnoremap <buffer> <c-]> :python trac.wiki_view ("<C-R><C-W>")<cr>')
        vim.command('cnoremap <buffer> q!<cr> python trac.uiwiki.tocwindow.resize_width(30)<cr>')
        #map gf to a new buffer (switching buffers doesnt work with nofile)
        vim.command('nnoremap <buffer> gf <c-w><c-f><c-w>K')
        vim.command('vertical resize +70')
        vim.command('setlocal syntax=wiki')
...
class TicketWindow (NonEditableWindow):
    ...
    def on_create(self):
        ...
        #vim.command('cnoremap <buffer> w<cr> TracSaveTicket<cr>')
        #vim.command('cnoremap <buffer> wq<cr> TracSaveTicket<cr>TracNormalView<cr>')
        vim.command('cnoremap <buffer> q<cr> python trac.normal_view()<cr>')
        #map gf to a new buffer (switching buffers doesnt work with nofile)
        vim.command('nnoremap <buffer> gf <c-w><c-f><c-w>K')
        #vim.command('setlocal linebreak')
...
class TicketCommentWindow (VimWindow):
    ...
    def on_create(self):
        vim.command('cnoremap <buffer> w<cr> python trac.ticket.add_comment()<cr>')
        vim.command('cnoremap <buffer> wq<cr> python trac.ticket.add_comment()<cr>python trac.normal_view()<cr>')
        vim.command('cnoremap <buffer> q<cr> python trac.normal_view()<cr>')
        vim.command('setlocal syntax=wiki')
        vim.command('setlocal noswapfile')
